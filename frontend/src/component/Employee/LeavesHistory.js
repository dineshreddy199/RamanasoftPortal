...
const handleEdit = (employee) => {
  navigate('/Employee/LeavesHistory/ApplyLeave', { state: { employee } });
};
  ...
  .filter(contact =>
    ...
    contact.endDate?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    contact.startDate?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <>
      ...
      <div className='Nav-txt'>Employee Leaves History</div>

      <div>
        <Link to="/Employee/LeavesHistory/ApplyLeave">
          <button
            type="button"
          ...
          {currentContacts.length === 0 ? (
            ...
            currentContacts.map((contact, index) => (
              <div className='TableBody-In' key={contact._id}>
                ...
                <span>{contact.startDate}</span>
                <span>{contact.endDate}</span>
