...
const LeaveReqest = () => {
  const API_BASE_URL=process.env.REACT_APP_URL+'/leaves';
  ...
  const handleEdit = (employee) => {
    navigate('/SuperAdmin/Approvels/LeaveRequest/AddLeaveRequest', { state: { employee } });
  };
  ...
  return (
    <>
      ...
      <div className='Nav-txt'>List Of Leave Details</div>
          ...
          {currentContacts.length === 0 ? (
            ...
            currentContacts.map((contact, index) => (
              <div className='TableBody-In' id='EmployeeReq' key={contact._id}>
                ...
                <span style={{marginLeft:'5px',width:'120px',textAlign:'center'}}>
                  {contact.status === 'Approved' || contact.status === 'Rejected' ? (
                    <span style={{ color: contact.status === 'Approved' ? 'green' : 'red', fontWeight: 'bold',marginRight:'-0.21rem' }}>
                      {contact.status}
                    </span>
                  ) : (
                    <>
                      <button onClick={() => askForConfirmation(contact._id, 'Rejected')} style={{ marginRight: '0.35rem' }}>
                        Reject
                      </button>
