...
const HrLeave = () => {
  const { user } = useAuth();
  const API_BASE_URL=process.env.REACT_APP_URL+'/hrLeave';
...
const handleEdit = (employee) => {
  navigate('/Admin/LeaveDetails/HrLeave/AddHrLeave', { state: { employee } });
};
  ...
  .filter(contact =>contact.employeeId === user.employeeId) // Filter only current user's records
  .filter(contact =>
  ...
  return (
    <>
      ...
      <div className='Nav-txt'>List Of Hr Leave Details</div>

      <div>
        <Link to="/Admin/LeaveDetails/HrLeave/AddHrLeave">
          <button
            type="button"
        ...
        <div className='TableBody'>
          {currentContacts.length === 0 ? (
            ...
            currentContacts.map((contact, index) => (
              <div className='TableBody-In' key={contact._id}><span >{startIndex + index + 1}</span>
          {/* {(contact.employeeId===`${user.employeeId}`)(<> */}
                <span>{contact.employeeId}</span>
                <span>{contact.startDate}</span>
                <span>{contact.endDate}</span>
                <span style={{width:"210px"}}>{contact.reason}</span>
